...
RSpec.describe 'munchies api' do
  describe 'munchies happy paths' do
    it 'sends recommendation info', :vcr do
      ...
      expect(recommendation[:data][:attributes][:forecast][:temperature]).to be_a(Numeric)

      expect(recommendation[:data][:attributes][:restaurant]).to have_key(:name)
      expect(recommendation[:data][:attributes][:name]).to be_a(String)
      expect(recommendation[:data][:attributes][:restaurant]).to have_key(:address)
